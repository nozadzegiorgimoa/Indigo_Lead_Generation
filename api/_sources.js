...
const SOURCE_RULES = [
  ...
  { canonical: 'Facebook',       re: /(?<![\p{L}])(facebook|fb)(?![\p{L}])|(?<![\p{L}])ფეისბუ/iu },
  ...
  { canonical: 'TikTok',         re: /(?<![\p{L}])(tik\s*tok|tiktok)(?![\p{L}])|(?<![\p{L}])ტიქტოქ/iu },
...
function stripSourceMentions(text) {
  ...
  for (const rule of SOURCE_RULES) {
    ...
    const src = rule.re.source.replace(/(ფეისბუ|ინსტა|ვოთსაფ|ვაცაპ|ვაიბერ|ტიქტოქ|ზარი|დარეკ|ადგილზე|რეკომენდაცი|მეგობ|საიტ|ლიდ\\s\*ფორმა|ლიდფორმა|ფორმა)/g, '$1\\p{L}*');
